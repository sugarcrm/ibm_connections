...
function showMemberProfile(member_id) {
    ...
    YAHOO.connections.memberProfile.cfg.applyConfig({
        width: "500px",
        visible: false,
        draggable: false,
        close: true,
        context: [member_id, "tl", "tr", ["windowResize"], [20, -10]]
    }, false);
...
function selectCommunity() {
    ...
    window.community_id = event.currentTarget.getAttribute('data-id');
    if (!_.isUndefined(window.community_id)) {
...
function quickCreateIBM(event) {
    if (event.keyCode == 13) {
        ...
        switch (asset_type) {
            case 'update':
                var url = createURL('method=postNote&content=' + ibm_name);
                break;
            case 'activity':
                var url = createURL('method=saveActivity&activity_name=' + ibm_name);
                break;
            case 'question':
                var url = createURL('method=saveDiscussion&discussion_name=' + ibm_name + '&discussion_is_question=1');
                break;
            case 'file':
                YAHOO.util.Connect.setForm('QuickpostFile', true);
                var url = createURL();
                post_data = 'method=uploadNewFile&file_name=' + ibm_name + '&visibility=private&quickpost=1';
                break;
            case 'bookmark':
                var url = createURL('method=saveBookmark&bookmark_path=' + ibm_name + '&bookmark_name=' + ibm_name.substr(0, 30));
                break;
            default :
                return false;
...
function loadTabData(tab_name, page, search_text)//method)
{
    ...
    switch (tab_name) {
        case 'Updates' :
            document.getElementById('ibm_asset').selectedIndex = 0;
            selectQuickCreateElement();
            break;
        case 'Activities' :
            document.getElementById('ibm_asset').selectedIndex = 1;
            selectQuickCreateElement();
            break;
        case 'Discussions' :
            document.getElementById('ibm_asset').selectedIndex = 2;
            selectQuickCreateElement();
            break;
        case 'Files' :
            document.getElementById('ibm_asset').selectedIndex = 3;
            selectQuickCreateElement('file');
            break;
        case 'Bookmarks' :
            document.getElementById('ibm_asset').selectedIndex = 4;
            selectQuickCreateElement();
            break;
    }
    SUGAR.ajaxStatusClass.prototype.showStatus(getLabel('LBL_LOADING'));
