...
class ConnectionsHelper
{
    ...
    public function getCommunityNavigation($communityId, $recordOwner)
    {
        ...
        if (empty($communityId)) {
            $head = ($recordOwner) ? $this->view->navWrapStart() . $this->view->selectCommunityMenu(
                ) . $this->view->navWrapEnd() : false;
        } else {
            $community = $this->apiClass->getCommunity($communityId);
            if (empty($community)) {
                $head = ($recordOwner) ? $this->view->navWrapStart() . "	{$this->view->actionsEditCommunity(
                        ''
                    )}" . $this->view->selectCommunityMenu() . $this->view->navWrapEnd() : '';

                return array(
                    'head' => "<div id='community_name'>{$this->language['LBL_PRIVATE_COMMUNITY']}</div>{$head}",
                    'visibility' => 'denied',
                    'members' => array()
                );
            }
            if ($community == "no_connection") {
                return array(
                    'head' => "<div id='community_name'>{$this->language['LBL_NO_CONNECTION']}</div>",
                    'visibility' => 'denied',
                    'members' => array()
                );
            }
            ...
            $navigation = ($recordOwner) ? $this->view->navWrapStart() . "	<li>{$this->view->actionsEditCommunity(
                    $communityId,
                    $isCommunityOwner
                )}</li>" . $this->view->selectCommunityMenu() . $this->view->navWrapEnd() : "";
            $head = "
			<div id='community_logo'>
					...
					<div id='community_name'> 
						...
						<a href='" . urldecode(
                    (string)$community->getLink()
                ) . "' target='_blank'>" . $community->getTitle() . "</a>
					</div>
					{$navigation}";
    ...
    private function getTabButtons()
    {
        switch ($this->tab_name) {
            case 'Discussions' :
                return '<div class="ibm_buttons">' . $this->view->button(
                    'LBL_NEW_DISCUSSION_BUTTON',
                    'createIBMElement("Discussion");'
                ) . "</div>";
                break;
            case 'Activities' :
                return '<div class="ibm_buttons">' . $this->view->button(
                    'LBL_NEW_ACTIVITY_BUTTON',
                    'createIBMElement("Activity");',
                    'newActivity'
                ) . "</div>";
                break;
            case 'Bookmarks' :
                return '<div class="ibm_buttons">' . $this->view->button(
                    'LBL_NEW_BOOKMARK_BUTTON',
                    'createIBMElement("Bookmark");'
                ) . "</div>";
                break;
            case 'Members' :
                return ($this->isCommunityOwner) ? '<div class="ibm_buttons">' . $this->view->button(
                        'LBL_ADD_MEMBER_BUTTON',
                        'createIBMElement("Member");'
                    ) . "</div>" : "";
                break;
        }
        return;
    ...
    {
        if ($relArray['value'] == 1 && $relArray['unit'] == 'DAY') {
            return $this->language['LBL_UNIT_YESTERDAY'];
        } else {
            return $relArray['value'] . " " . $this->language['LBL_UNIT_' . $relArray['unit']] . " " . $this->language['LBL_AGO'];
        }
    }
    ...
    public function saveBookmark()
    {
        ...
        $this->apiClass->createBookmarkToCommunity(
            $this->getCommunityId(),
            $this->bookmark_name,
            $href,
            $this->bookmark_description,
            $tag,
            $this->bookmark_important
        );
    ...
    public function saveActivity()
    {
        ...
        if (!empty($this->due_date)) {
            $this->due_date = $timedate->asDbDate(
                $timedate->fromUserDate($this->due_date)
            ); ///$timedate->convert_to_gmt_datetime($this->due_date);
        }
    ...
    public function saveActivityToDo()
    {
        ...
        if (!empty($this->ibm_section_id)) {
            $this->apiClass->createSectionToDo(
                $this->ibm_parent_id,
                $this->ibm_section_id,
                $this->title,
                $this->description,
                $this->due_date,
                $tag,
                array('name' => $this->assigned_name, 'id' => $this->assigned_id)
            );
        } else {
            $this->apiClass->createActivityToDo(
                $this->ibm_parent_id,
                $this->title,
                $this->description,
                $this->due_date,
    ...
    public function saveActivityEntry()
    {
        ...
        if (!empty($this->ibm_section_id)) {
            $this->apiClass->createSectionEntry(
                $this->ibm_parent_id,
                $this->ibm_section_id,
                $this->title,
                $this->description,
    ...
    public function saveDiscussion()
    {
        ...
        $this->apiClass->createDiscussion(
            $this->getCommunityId(),
            $this->discussion_name,
            $this->discussion_content_form,
            $tag,
            $this->discussion_is_question
        );
    }
    ...
    public function loadUpdatesList()
    {
        ...
        ob_clean();
        echo json_encode(
            array('frame' => $reply, 'content' => $list, 'container_id' => 'update_list', 'page' => $this->page_number)
        );
    }
    ...
    public function saveCommunity()
    {
        ...
        if (!empty($this->ibm_id)) {
            ...
            $communityId = $this->apiClass->createCommunity(
                $this->community_name,
                $this->description,
                $this->access,
                $tags,
    ...
    public function saveSubcommunity()
    {
        global $current_user;
        $tags = explode(",", $this->community_tags);
        $subcommId = $this->apiClass->createSubcommunity(
            $this->ibm_id,
            $this->community_name,
            $this->description,
            $this->access,
    ...
    public function loadFilesList()
    {
        ...
        if (empty($community)) {
            ...
            echo json_encode(
                array(
                    'frame' => $reply,
                    'content' => $this->language['NO_CCONNECTIONS'],
                    'container_id' => $tab . '_list'
                )
            );
            return;
        }
    ...
    public function loadCreationBody()
    {
        ...
        if (isset($this->ibm_user_id) && !empty($this->ibm_user_id)) {
            $smarty->assign('user_id', $this->ibm_user_id);
        }
        if (isset($this->ibm_doc_id) && !empty($this->ibm_doc_id)) {
            $smarty->assign('document_id', $this->ibm_doc_id);
        }
        ...
        if (isset($this->community_id) && !empty($this->community_id)) {
            $smarty->assign(
                'community_id',
                $this->community_id
            );
        ...
        if (isset($this->reply_to) && !empty($this->reply_to)) {
            $smarty->assign('reply_to', $this->reply_to);
        }
        if (isset($this->ibm_parent_id) && !empty($this->ibm_parent_id)) {
            $smarty->assign(
                'ibm_parent_id',
                $this->ibm_parent_id
            );
        }
        if (isset($this->ibm_section_id) && !empty($this->ibm_section_id)) {
            $smarty->assign(
                'ibm_section_id',
                $this->ibm_section_id
            );
        }
        if (isset($this->ibm_discussion_id) && !empty($this->ibm_discussion_id)) {
            $smarty->assign(
                'ibm_discussion_id',
                $this->ibm_discussion_id
            );
        }
    ...
    public function loadEditionBody()
    {
        ...
        $smarty->assign(
            'type_description',
            $this->language["LBL_" . strtoupper($community->getCommunityType()) . "_ACCESS_DESCRIPTION"]
        );
    ...
    function getDiscussion()
    {
        ...
        $reply = $this->view->button(
            'LBL_REPLY',
            'createIBMElement("DiscussionReply","&ibm_parent_id=' . $forumId . '&reply_to=topic");'
        );
