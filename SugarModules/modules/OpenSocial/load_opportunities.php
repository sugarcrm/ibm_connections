...
class OpportunityLoader {
	...
	protected function loadOpportunities() {
		...
		$opportunities = array();
		$opportunities = $bean->get_full_list("date_closed", "opportunities.assigned_user_id = '" . $this->user->id . "'");
		$this->opportunities = array();
		foreach ($opportunities as $opportunity) {
		  //$GLOBALS['log']->fatal($opportunity->id);
	...
	private function contactsToJSON(Opportunity $opportunity) {
	...
	private function OpportunityToJSON(Opportunity $opportunity) {
		...
		return array(
			...
			//"contacts"  => $this->contactsToJSON($opportunity), 
		);
	}
}
