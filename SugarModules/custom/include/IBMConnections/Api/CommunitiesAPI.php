...
class CommunitiesAPI extends AbstractConnectionsAPI {
    ...
    public function editCommunity($communityId, $title = null, $content = null, $type = null, $tags = array()) {
       ...
       // $client = $this->getHttpClient();
        ...
        $response = $this->requestForPath('PUT',
     ...
     */
    public function listMyCommunities($search = null, $filterType = null, $sortBy = null, $sortOrder = null, $pageSize = null, $pageNumber = null) {
        $this->getHttpClient()->resetParameters();
        if ($filterType != null) {
     ...
     * @return multitype:ConnectionsCommunity
     */
    public function listAllCommunities($search = null, $filterType = null, $sortBy = null, $sortOrder = null, $pageSize = null, $pageNumber = null) {
        $this->getHttpClient()->resetParameters();
        if ($filterType != null) {
        	$this->getHttpClient()->setParameterGet("filterType", $filterType);
        }
    ...
    public function activateActivities($communityId) {
        $this->activateAppWidget($communityId, "Activities");
    }

    /**
     *
     * Enter description here ...
     * @param unknown_type $communityId
     */
    public function activateWiki($communityId) {
        $this->activateAppWidget($communityId, "Wiki");
    }
     ...
     * @param unknown_type $communityId
     */
    public function activateBlog($communityId) {
        $this->activateAppWidget($communityId, "Blog");
    }
     ...
     */
    protected function activateAppWidget($communityId, $appName) {
        $client = $this->getHttpClient(true);
        $activationHeaders = array('Pragma' => 'WWW-Authenticate=XHR',
                'X-Requested-With' => 'XMLHttpRequest',
                'Content-Type' => 'application/x-www-form-urlencoded');

        $client->setHeaders($activationHeaders);

        $client->setParameterGet("resourceId", $communityId);
        $client->setParameterGet("uiLocation", "col2");
        $client->setParameterGet("unhide", "false");
        $client->setParameterGet("widgetDefId", $appName);

        $response = $this
                ->requestForPath('POST',
                        '/communities/addWidget.do?resourceId=' . $communityId);

        /*if ($response->getStatus() != 200) {
            throw new IBMConnectionsServiceException(
                    "Failed to Activate $appName App widget: "
                            . $response->getStatus(),
        ...
        if ($members && !empty($members)) {
            ...
            $existingRole = $dom
                    ->getElementsByTagNameNS(
                            'http://www.ibm.com/xmlns/prod/sn', 'role')
                    ->item(0);
    ...
    function addMember($cid, $member) {
    	...
    	// New role.
    	$roleElt = $dom->createElementNS('http://www.ibm.com/xmlns/prod/sn', 'snx:role',
    ...
    public function AddMemberById($member_id, $community_id, $role)
    {
		...
		$this->getHttpClient()->resetParameters();		
		$this->getHttpClient()->setRawData($entry->getDomString());
		$result = $this->requestForPath('POST', "communities/service/atom/community/members?communityUuid={$community_id}",
			array(
    ...
    public function shareMyFileWithCommunity($cid, $myFileId) {
       ...
       // echo $response->getStatus();
        ...
        $response = $this
                ->requestForPath('POST', '/files/basic/api/myuserlibrary/document/' . $myFileId . '/feed',
